...
begin
   ...
   if value = EmptyStr then
    raise Exception.Create('ERRO! Informe o nome do funcionário');
...
begin
   ...
   if value = 0 then
    raise Exception.Create('ERRO! Informe código funcionário');
...
begin
  ...
  begin
    ...
    FieldByName('FORMRELATVENDASFUNCIONARIO').DisplayLabel := 'Rel. Vendas por funcionário';
    FieldByName('FORMRELATTRANSPORTADORA').DisplayLabel := 'Relatório transportadora';
    ...
    FieldByName('FORMCOMISSOESTECNICOS').DisplayLabel := 'Comissões técnicos';
    FieldByName('FORMCONFIGCONEXAOBANCO').DisplayLabel := 'Configurar conexão';
    ...
    FieldByName('FORMMOVIMENTACAOCAIXA').DisplayLabel := 'Movimnetação do caixa';
