...
begin
  ...
  FGravarLog.getJanela('Itens da ordem de serviço').getCodigoFuncionario
    (funcionarioLogado);
...
begin
  result := self;
  if value = 0 then
    raise Exception.create('Você deve informar o produtos que esta inserindo.');
  FID_PRODUTO := value;
end;
...
begin
  ...
  try
    ...
    begin
      raise Exception.create
        ('Informe um valor válido para o valor do produto.');
    end;
...
begin
  ...
  try
    ...
    begin
      raise Exception.create
        ('informe um valor válido para o total do produto.');
    end;
