...
begin
  ...
  if edtHost.Text = EmptyStr then
    raise Exception.Create('ERRO. Informe um endereço de ip válido');

  if edtLocalDoBanco.Text = EmptyStr then
     raise Exception.Create('ERRO. Informe a localização do banco de dados que irá utilizar');

  if edtUserName.Text = EmptyStr then
     raise Exception.Create('ERRO. Informe o usuário padrão do sistema');

  if edtPassword.Text = EmptyStr then
     raise Exception.Create('ERRO. Informe a senha de conexão com o banco de dados');

  if edtPort.Text = EmptyStr then
    raise Exception.Create('ERRO. Informe a  porta de conexão com o banco de dados');
